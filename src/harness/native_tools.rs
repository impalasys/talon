...
async fn prepare_code_mounts(
    ...
    for (index, mount) in requested.iter().enumerate() {
        ...
        mounted_inputs.push(json!({
            "uri": uri,
            "mountPath": format!("{CODE_INPUT_MOUNT}/{}", mount_path.display()),
...
async fn persist_code_outputs(
    ...
    for relative in paths {
        ...
        let artifact_output = create_artifact(
            ...
            &json!({
                ...
                "metadata": {
                    ...
                    "mountPath": format!("{CODE_OUTPUT_MOUNT}/{}", relative.display())
                }
            }),
        )
        .await?;
        ...
        outputs.push(json!({
            "path": format!("{CODE_OUTPUT_MOUNT}/{}", relative.display()),
            "sizeBytes": bytes.len(),
            "mediaType": media_type,
...
fn monty_object_to_json(value: &monty_types::MontyObject) -> Result<Value> {
    Ok(match value {
        ...
        monty_types::MontyObject::Float(value) => json!(value),
