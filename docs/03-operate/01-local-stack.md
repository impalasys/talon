...
Run the complete Talon development environment when you need Sightline, Postgres-backed state, Pub/Sub dispatch, and manifest bootstrap.

## Local topology

Running the Docker Compose stack starts:

- Sightline UI on `http://localhost:3000`
- gateway with native gRPC and gRPC-Web on `http://localhost:50051`
- worker execution
- Postgres and the Pub/Sub emulator
- manifest and API-key bootstrap jobs

The browser talks to Sightline and the gateway's gRPC-Web surface. Backend integrations use the gateway's native gRPC service. When a client submits a session message, the gateway persists it and publishes dispatch work; the worker executes the turn and persists messages and step events for the gateway and UI to read.

The bootstrap job applies `manifests/default`, so the stack begins with `Namespace/default`, `Template/default`, and `Agent/default/main`.
...
The Docker Compose stack uses Postgres and the Pub/Sub emulator. For the smallest local runtime, use `talon-node` with SQLite and a local Unix socket broker, as shown in the [Quickstart](../build/quickstart).
