...
import { AssistantTimeline, coalesceAssistantTimelineForDisplay, splitFinalAssistantTimeline } from "./AssistantTimeline";
...
> & {
  items: ReturnType<typeof splitFinalAssistantTimeline>["workTimeline"];
  isLive: boolean;
...
function WorkDetailsContent({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 12 }}>
      <AssistantTimeline message={message} items={items} variant="work" isLive={isLive} expandedTools={expandedToolItems} hydrationState={hydrationState} resultFor={resultFor} onToggleTool={onToggleTool} onHydrateTool={onHydrateTool} onResourceClick={onResourceClick} />
...
function MessageWorkDetails({
  ...
  const workTimeline = splitFinalAssistantTimeline(timeline).workTimeline;
...
function MessageContent({
  ...
  const timeline = splitFinalAssistantTimeline(
    coalesceAssistantTimelineForDisplay(getMessageAssistantTimeline(message)),
  ).finalTimeline;
  return (
    <div
    ...
    >
      {message.role === "assistant" && timeline.length > 0 ? (
        <AssistantTimeline message={message} items={timeline} variant="final" isLive={isLiveAssistantMessage} expandedTools={expandedToolItems} hydrationState={hydrationState} resultFor={resultFor} onToggleTool={onToggleTool} onHydrateTool={onHydrateTool} onResourceClick={onResourceClick} />
